...
(() => {
  ...
  async function fetchRecentGames(username) {
    ...
    if (!archivesRes.ok) {
      if (archivesRes.status === 404) throw new Error(`No Chess.com account found for "${username}".`);
      throw new Error(`Chess.com API returned ${archivesRes.status} while fetching archives.`);
    }
    const archivesData = await archivesRes.json();
    const archives = archivesData.archives || [];
    if (archives.length === 0) throw new Error(`"${username}" has no game archives.`);
    ...
    for (let i = archives.length - 1; i >= 0 && monthsChecked < 6 && collected.length < 10; i--) {
      ...
      const games = (data.games || []).filter(g => g.rules === "chess"); // standard chess only
  ...
  dom.form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      state.games = games;
      renderGameList();
      setStatus(games.length ? null : `No recent standard-chess games for "${username}".`, games.length ? null : "error");
    } catch (err) {
      setStatus(err.message || "Something went wrong.", "error");
      dom.gameList.innerHTML = `<div class="empty-state"><p>Couldn't load games. ${escapeHtml(err.message || "")}</p></div>`;
    } finally {
